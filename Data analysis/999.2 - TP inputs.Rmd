...
min = min(tp_input_trend_flux$trend)*10
max = max(tp_input_trend_flux$trend)*10
abs_max = max(abs(c(min,max)))

for_states <- tp_input_trend_flux%>%
  left_join(lat_long)
...
min = min(tp_input_trend_conc$trend)*10
max = max(tp_input_trend_conc$trend)*10
abs_max = max(abs(c(min,max)))

for_states <- tp_input_trend_conc%>%
  left_join(lat_long)
states <- ne_states(returnclass = "sf",country = "United States of America")

us_map_conc = ggplot(data = states) +
  geom_sf(fill = "white") +
  coord_sf(expand = FALSE, ylim = c(42, 47.5), xlim = c(-93,-86.5))+
  geom_point(data = for_states, 
             aes(Longitude_DD, Latitude_DD, fill = trend*10),
...
ggarrange(us_map_flux, us_map_conc)
dev.off()
```
